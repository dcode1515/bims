      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
          <div class="modal-body text-center p-5">
            ...
            <p class="text-muted mb-0">Please wait while we process your household registration...</p>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
        ...
        return this.formData.householdInfo.livestockRaised
          .map(livestock => {
            if (livestock.category === "Poultry" && livestock.details) {
              return livestock.details.map(d => `${d.subType} (${d.quantity} ${d.unit})`).join(", ");
            }
    ...
    removeOtherCrop(index) {
     ...
     toggleAllLivestock() {
      if (this.areAllLivestockSelected) {
        ...
        this.selectedLivestock = this.livestockOptions.map(animal => animal.value);
    ...
    prepareHouseholdData() {
      ...
      if (this.selectedLivestock.includes("Poultry")) {
        ...
        // Add other poultry
        if (this.poultryType.includes("Other Poultry") && 
      ...
      if (category === "headOfFamily") {
        ...
        switch (field) {
          ...
          case "garbageDisposal":
            if (!value)
              this.fieldErrors[fieldPath] = "Garbage disposal method is required";
            break;
        }
      }
    },
    ...
    async submitForm() {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (!this.validateCurrentStep()) {
          ...
          this.currentStep = i;
          this.validationErrors = ["Please fix all validation errors before submitting."];
          window.scrollTo({ top: 0, behavior: 'smooth' });
          return;
        }
      }
      ...
      try {
        ...
        if (data.success) {
          ...
          this.referenceId = data.data.reference_id || this.generateReferenceId();
        ...
        // Show error message
        this.validationErrors = [error.message || 'An error occurred during submission. Please try again.'];
        window.scrollTo({ top: 0, behavior: 'smooth' });
      } finally {
        // Reset loading state
