            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <div class="row align-items-center mb-4 g-3">
                        ...
                        <div class="col-md-3">
                            <div class="input-group">
                                ...
                                <select class="form-control" v-model="perPage" @change="getDataBuildingPermit">
                        ...
                        <div class="col-md-4">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" v-model="searchQuery"
                                    @input="getDataBuildingPermit" placeholder="Search name..." />
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0 shadow-sm rounded">
                            <thead class="table-primary bg-opacity-10">
                                <tr>
                                    ...
                                    <th><i class="ri-file-list-line me-1"></i> Building Permit #</th>
                            ...
                            <tbody>
                                <tr v-for="(building, index) in buildings.data" :key="building.id">
                                    <td class="ps-4 fw-medium">
                                        {{
                                            (buildings.current_page - 1) * buildings.per_page +
                                            index +
                                            1
                                        }}
                                    </td>
                                    <td>{{ building.building_permit_no }}</td>

                                    <td>{{ building.requestor.first_name }} {{ building.requestor.middle_initial }}.
                                        {{
                                            building.requestor.last_name }}</td>
                                    <td>{{ building.purpose }}</td>

                                    <td>{{ building.purok.purok_name }}</td>
                                    <td>{{ formatDate(building.date_issued) }}</td>
                        ...
                        <div v-if="buildings.total > 0" class="card-footer bg-white">
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <span class="text-muted small">
                                        <i class="ri-file-list-line me-1"></i>
                                        Showing {{ buildings.from }} to {{ buildings.to }} of
                                        {{ buildings.total }} entries
                                    </span>
                                </div>
                                <div class="col-md-6">
                                    <nav class="float-end">
                                        <ul class="pagination pagination-sm mb-0">
                                            <li class="page-item" :class="{ disabled: buildings.current_page === 1 }">
                                            ...
                                            <li class="page-item" :class="{ disabled: buildings.current_page === 1 }">
                                                <button class="page-link"
                                                    @click="changePage(buildings.current_page - 1)" title="Previous">
                                                    <i class="ri-arrow-left-s-line"></i>
                                                </button>
                                            </li>

                                            <li v-for="page in pages" :key="page" class="page-item"
                                                :class="{ active: page === buildings.current_page }">
                                            ...
                                            <li class="page-item" :class="{
                                                ...
                                                <button class="page-link" @click="changePage(puroks.last_page)"
                                                    title="Last">
                        ...
                        <div class="modal fade zoomIn" id="modalBuilding" tabindex="-1"
                                    ...
                                    <div class="modal-body p-4">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-12">
                                                    <label class="form-label fw-medium">
                                                        <i class="ri-map-pin-line me-1 text-muted"></i> Enter Case
                                                    </label>
                                                    <textarea v-model="formData.case" class="form-control"
                                                        name="purpose" rows="3"
                                                        placeholder="Enter purpose here..."></textarea>
                                                    <small class="text-muted">
                                                        <i class="ri-information-line"></i> Please Enter if you have
                                                        Cases in this barangay
                                                    </small>
                                                </div>
                                                <div class="col-md-12">
                                                    <label class="form-label fw-medium">
                                                        <i class="ri-map-pin-line me-1 text-muted"></i> Requirements for
                                                        Building Permit
                                                    </label>
                                                    <textarea v-model="formData.requirements_building"
                                                        class="form-control" name="building_permit_requirements"
                                                        rows="3"
                                                        placeholder="Enter details of your requirements here..."></textarea>
...
export default {
    ...
    methods: {
        ...
        changePage(page) {
            if (page >= 1 && page <= this.buildings.last_page) {
                this.buildings.current_page = page;
                this.getDataBuildingPermit();
            }
        },
        ...
        openModal(mode, building = null) {
            this.modalMode = mode;

            if (mode === "add") {
                this.modalTitle = "Add Building Permit";
                this.formData = {
                    name_requestor: "",
                    purok: "",
                    purpose: "",
                ...
                this.modalTitle = "Edit Building Permit";
                this.formData = {
                    ...building,
                    name_requestor: building.requestor?.id || building.name_requestor,
                    purok:
                        building.purok && building.purok.id
                            ? building.purok.id
                            : building.purok,
                    date_issued: building.date_issued
                        ? new Date(building.date_issued).toISOString().substr(0, 10)
                        : "",
                };
            }
    ...
    data() {
        return {
            ...
            loading: false,
            modalMode: "add",
            modalTitle: "Add Building Permit",
            formData: {
                name_requestor: null,
                purok: "",
                date_issued: "",
    ...
    computed: {
        ...
        pages() {
            const pages = [];
            const total = this.buildings.last_page;
            const current = this.buildings.current_page;
            const maxVisible = 5;
