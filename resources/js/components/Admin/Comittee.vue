    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card border-0 shadow-sm">
                    ...
                    <div class="row g-3 align-items-center">
                        <div class="col-md-3">
                            <div class="input-group input-group">
                                ...
                                <select class="form-control form-control" v-model="perPage" @change="getDataCommitee">
                        ...
                        <div class="col-md-6">
                            <div class="input-group input-group">
                                ...
                                <input v-model="searchQuery" @input="getDataCommitee" type="text" class="form-control"
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                <tr v-for="(commitee, index) in committees.data" :key="commitee.id">
                                    <td class="ps-4">
                                        {{
                                            (committees.current_page - 1) * committees.per_page + index + 1
                                        }}
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                    ...
                                    <td>
                                        <div>
                                            {{ formatDate(commitee.created_at) }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <button class="btn btn-sm btn-outline-primary"
                                                @click="openModal('edit', commitee)" title="Edit">
                                                <i class="ri-edit-line"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-info" @click="viewDetails(commitee)"
                                                title="View">
                                                <i class="ri-eye-line"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger"
                                                @click="confirmDelete(commitee)" title="Delete">
                        ...
                        <div class="modal fade zoomIn" id="modalCommitee" tabindex="-1"
                ...
                <!-- Pagination -->
                <div v-if="committees.total > 0" class="card-footer bg-white">
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <span class="text-muted small">
                                <i class="ri-file-list-line me-1"></i>
                                Showing {{ committees.from }} to {{ committees.to }} of
                                {{ committees.total }} entries
                            </span>
                        </div>
                        <div class="col-md-6">
                            <nav class="float-end">
                                <ul class="pagination pagination-sm mb-0">
                                    <li class="page-item" :class="{ disabled: committees.current_page === 1 }">
                                        <button class="page-link" @click="changePage(1)" title="First">
                                            <i class="ri-skip-back-line"></i>
                                        </button>
                                    </li>
                                    <li class="page-item" :class="{ disabled: committees.current_page === 1 }">
                                        <button class="page-link" @click="changePage(committees.current_page - 1)"
                                            title="Previous">
                                    ...
                                    <li class="page-item" :class="{
                                        disabled: committees.current_page === committees.last_page,
                                    }">
                                        <button class="page-link" @click="changePage(committees.current_page + 1)"
                                            title="Next">
                                    ...
                                    <li class="page-item" :class="{
                                        ...
                                        <button class="page-link" @click="changePage(committees.last_page)"
                                            title="Last">
...
export default {
    ...
    computed: {
        pages() {
            const pages = [];
            const total = this.committees.last_page;
            const current = this.committees.current_page;
            const maxVisible = 5;
    ...
    methods: {
        ...
        changePage(page) {
            if (page >= 1 && page <= this.committees.last_page) {
                this.committees.current_page = page;
        ...
        openModal(mode, commitee = null) {
            this.modalMode = mode;

            if (mode === "add") {
                this.modalTitle = "Add Commitee";
                this.formData = { id: "", commitee: "", status: "active" };
            } else if (mode === "edit") {
                this.modalTitle = "Edit Commitee";
                this.formData = {
                    id: commitee.id,
                    commitee: commitee.commitee,
                    status: commitee.status || "active",
                };
            }

            $("#modalCommitee").modal("show");
        },
        closeModal() {
            $("#modalCommitee").modal("hide");
        },

        viewDetails(commitee) {
            this.modalMode = "view";
        ...
        async submitForm() {
            ...
            try {
                ...
                $("#modalCommitee").modal("show");
                if (this.modalMode === "add") {
                    this.formData.commitee = "";
                }

                this.getDataCommitee();
            } catch (error) {
                if (error.response?.status === 409) {
                    this.showError("This Committee  already exists");
