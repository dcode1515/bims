...
<div class="content-container">
  ...
  @if(Auth::user()->barangay->photo)
    ...
    <p class="paragraph-text">
        This is to certify that <b>{{ $indigency_permit->requestor->first_name }}
        {{ $indigency_permit->requestor->middle_initial }}.
        {{ $indigency_permit->requestor->last_name }}</b>, legal age, Filipino citizen, 
        with residence at <b>{{ $indigency_permit->purok->purok_name }}</b>, 
        {{ Auth::user()->barangay->barangay_name }}, {{ Auth::user()->barangay->municipality->municipality_name }}, 
        whose specimen signature appears below, is an <b>INDIGENT</b> and that she/he has visibly no money, 
        property or means of livelihood sufficient and available for daily food, shelter and basic necessities 
        for himself and his family.
    </p>

    <p class="paragraph-text">
        This certification is being issued upon the request of 
        <b>{{ $indigency_permit->requestor->first_name }} {{ $indigency_permit->requestor->middle_initial }}.
        {{ $indigency_permit->requestor->last_name }}</b> for <b>{{ $indigency_permit->purpose }}</b>.
    </p>
    ...
    <p class="paragraph-text">
        ...
        <b>{{ strtoupper($issuedDate->format('F')) }} {{ $issuedDate->format('Y') }}</b> 
        at {{ Auth::user()->barangay->barangay_name }}, {{ Auth::user()->barangay->municipality->municipality_name }}.
    </p>
