...
of legal age, a resident of @if(Auth::user()->barangay )@endif {{ Auth::user()->barangay->barangay_name }}, {{ Auth::user()->barangay->municipality->municipality_name }}, 
        has requested Barangay Clearance for the purpose of <b><u>{{$building_permit->purpose}}</b></u> 
        situated at {{ $building_permit->purok->purok_name }}, @if(Auth::user()->barangay )@endif {{ Auth::user()->barangay->barangay_name }}, {{ Auth::user()->barangay->municipality->municipality_name }}.
    </p>

    <p style="font-size:12pt; margin-left:12px; text-align:justify; font-family:'Bookman Old Style', serif;">
        After due verification and consideration, it is hereby certified that the said owner has complied 
        with all the pertinent requirements and regulations of  @if(Auth::user()->barangay )@endif {{ Auth::user()->barangay->barangay_name }}, {{ Auth::user()->barangay->municipality->municipality_name }}. 
        Furthermore, the applicant {{$building_permit->case}} in this Barangay.
    </p>

    <p style="font-size:12pt; margin-left:12px; text-align:justify;">
...
@endphp
    ...
    {{ $issuedDate->format('Y') }} </b> at @if(Auth::user()->barangay )@endif {{ Auth::user()->barangay->barangay_name }}, 
    ...
    <div class="blurred" style="margin-top:10px; margin-left:240px; font-family:'Bookman Old Style', serif;">
